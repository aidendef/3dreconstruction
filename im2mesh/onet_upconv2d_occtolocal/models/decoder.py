...
class DecoderCBatchNorm(nn.Module):
    ...
    def forward(self, p, z, v, G, **kwargs):
        ...
        G = G.repeat(1,T,1)
        G = G.transpose(1, 2)
        net = self.block0_1(net, G)
        net = self.block0_2(net, G)
        net = self.block0_3(net, G)
        net = self.block0_4(net, G)

        out_G = self.fc_out(self.actvn(self.bn(net, G)))
        out_G = out_G.squeeze(1)

        c1 = c1.transpose(1, 2)
        net = self.block1(net, c1)

        c2 = c2.transpose(1, 2)
        net = self.block2(net, c2)

        c3 = c3.transpose(1, 2)
        net = self.block3(net, c3)

        c4 = c4.transpose(1, 2)
        net = self.block4(net, c4)
