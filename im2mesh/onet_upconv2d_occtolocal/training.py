...
class Trainer(BaseTrainer):
    ...
    def compute_loss(self, data):
        ...
        # General points
        # logits_L, logits_G = self.model.decode(p, z, v, G, **kwargs).logits
        logits_L, logits_G = self.model.decode(p, z, v, G, **kwargs)
        logits_L = logits_L.logits
        logits_G = logits_G.logits
        # exit(1)
        loss_i_G = F.binary_cross_entropy_with_logits(
            logits_G, occ, reduction='none')

        loss_i_L = F.binary_cross_entropy_with_logits(
            logits_L, occ, reduction='none')
        loss = loss + (0.5*loss_i_L.sum(-1).mean()) + (0.5*loss_i_G.sum(-1).mean())
