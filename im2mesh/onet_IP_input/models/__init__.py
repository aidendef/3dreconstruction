...
from im2mesh.onet_IP_input.models import encoder_latent, decoder
from im2mesh.onet_IP_input.layers import GraphProjection
...
class OccupancyNetwork(nn.Module):
    ...
    def __init__(self, decoder, encoder=None, encoder_latent=None, encoder_2=None, encoder_2_latent=None, p0_z=None,
                 device=None):
        super().__init__()
        if p0_z is None:
    ...
    def gproj(self, p, c, world_mat, camera_mat, img_gproj=None, visualise_gproj=False):
        v = self.gp(p, c, world_mat, camera_mat, img=img_gproj, visualise=visualise_gproj)
        return v

    def forward(self, p, inputs, world_mat, camera_mat, sample=True, semantic_map=None, **kwargs):
        ...
        p_r = self.decode(p, z, v, **kwargs)
        return p_r

    def compute_elbo(self, p, occ, inputs, world_mat, camera_mat, semantic_map, **kwargs):
    ...
    def encode_inputs_2(self, inputs):
        ''' Encodes the input.
