...
class PointsField(Field):
    ...
    def __init__(self, file_name, transform=None, with_transforms=False, unpackbits=False):
        ...
        self.main_dir = '/home/hpclab/kyg/occupancy_networks'
    ...
    def load(self, model_path, idx, category):
        ...
        Args:
            ...
            category (int): index of category
        '''
        model_path = os.path.join(self.main_dir, model_path)
...
class ImagewithPointCloudField(Field):
    ''' Point cloud field.
    ...
    Args:
        ...
        with_transforms (bool): whether scaling and rotation dat should be
            provided
    '''
    def __init__(self, #folder_name_I=None, transform_I=None,
                 #extension_I='jpg', random_view_I=True, with_camera_I=False,
                 file_name_P=None, transform_P=None, with_transforms_P=False):
        
        # print("folder_name_I",folder_name_I)
        # exit(1)
        # self.folder_name_I = folder_name_I
        # self.transform_I = transform_I
        # self.extension_I = extension_I
        # self.random_view_I = random_view_I
        # self.with_camera_I = with_camera_I

        self.file_name_P = file_name_P
        self.transform_P = transform_P
        self.with_transforms_P = with_transforms_P

        self.main_dir = '/home/hpclab/kyg/occupancy_networks'
        
        # assert(self.folder_name_I != None)
        # assert(self.file_name_P != None)
        # assert(folder_name_I != None)
        assert(file_name_P != None)
    ...
    def load(self, model_path, idx, category):
        ...
        file_path_P = os.path.join(model_path, self.file_name_P)
        
        pointcloud_dict = np.load(file_path_P)
        ...
        # folder_I = os.path.join(model_path, self.folder_name_I)
        # files_I = glob.glob(os.path.join(folder_I, '*.%s' % self.extension_I))
        # files_I.sort()

        # if self.random_view_I:
        #     idx_img = random.randint(0, len(files_I)-1)
        # else:
        #     idx_img = 0
        # filename = files_I[idx_img]

        # image = Image.open(filename).convert('RGB')
        # # print("111")
        # if self.transform_I is not None:
        #     image = self.transform_I(image)

        # if self.with_camera_I:
        #     camera_file = os.path.join(folder_I, 'cameras.npz')
        #     camera_dict = np.load(camera_file)
        #     Rt = camera_dict['world_mat_%d' % idx_img].astype(np.float32)
        #     K = camera_dict['camera_mat_%d' % idx_img].astype(np.float32)
        #     data['world_mat'] = Rt
...
class imagewithpointcloud(Field):
    ''' Point cloud field.
    ...
    Args:
        ...
        with_transforms (bool): whether scaling and rotation dat should be
            provided
    '''
    def __init__(self, file_name_P, transform_P=None, with_transforms_P=False,
                 folder_name_I=None, transform_I=None,
                 with_camera_I=False, random_view_I=True, extension_I='jpg'):
        self.file_name_P = file_name_P
        self.transform_P = transform_P
        self.with_transforms_P = with_transforms_P
    ...
    def load(self, model_path, idx, category):
        ...
        Args:
            ...
            category (int): index of category
        '''
        file_path = os.path.join(model_path, self.file_name_P)
