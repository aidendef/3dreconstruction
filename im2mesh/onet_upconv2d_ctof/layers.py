...
class GraphProjection(nn.Module):
    ...
    def visualise_projection(self, points_img, img, output_file='/home/hpclab/kyg/occupancy_networks/test/gp_out.png'):
        ...
        # # print("points_img",np.shape(points_img))
        ...
        # plt.plot( points_img[:, 0],points_img[:, 1], 'x')
    ...
    def forward(self, x, fm, world_mat, camera_mat, img=None, visualise=False):
        ...
        points_img = common.project_to_camera_RT(x, world_mat, camera_mat)
        points_img = points_img.unsqueeze(1)
        # points_img_test = common.project_to_camera(x, world_mat)
        # points_img_test = points_img_test.unsqueeze(1)
        feats = []
        feats.append(x)
        for fmap in fm:
        ...
        if visualise and (img is not None):
            self.visualise_projection(
                ...
                img[0].cpu().numpy(),
                output_file='/home/hpclab/kyg/occupancy_networks/test/gp_wc1.png')
            # self.visualise_projection(
            #     points_img_test.squeeze(1)[0].detach().cpu().numpy(),
            #     img[0].cpu().numpy(),
            #     output_file='/home/hpclab/kyg/occupancy_networks/test/gp_w1.png')
