...
from im2mesh.onet_upconv2d.models import encoder_latent, decoder
from im2mesh.onet_upconv2d.layers import GraphProjection
...
class OccupancyNetwork(nn.Module):
    ...
    def gproj(self, p, G, c, world_mat, camera_mat, img_gproj=None, visualise_gproj=False):
        v = self.gp(p, c, world_mat, camera_mat, img=img_gproj, visualise=visualise_gproj)
        batch_size, pnn, ffm = v.size()
        G = G.unsqueeze(1)
        G = G.repeat(1,pnn,1)
    ...
    def forward(self, p, inputs, world_mat, camera_mat, sample=True, **kwargs):
        ...
        v = self.gproj(p, G, c, world_mat, camera_mat, inputs, False)
